...
bool Device::erroCode(int ret, IXML_Document *action, IXML_Document *response)
{
    if(ret != UPNP_E_SUCCESS)
	{
		...
		if(response)
        {
            ...
            IXML_Element* iElement = (IXML_Element*)ixmlNodeList_item(UPnPError, 0);

            Log::err("error code: %s desc: %s",
                   xml_getChildElementValue(iElement, "errorCode"),
                   xml_getChildElementValue(iElement, "errorDescription"));

            ixmlNodeList_free((IXML_NodeList*)iElement);
...
void Device::fetchInfo(void)
{
	...
	{
		Log::err("error: UpnpDownloadXmlDoc: %s", _Location.c_str());
		return;
	}
...
IXML_Document *Device::SendAction(ServiceType stype, const char* _actionName, int count, ...)
{
	...
	if(!srv)
    {
        Log::err("Service type %d not found", stype);
        return false;
    }

	char* url = strdup(srv->controlURL.c_str());
	char* urn = strdup(srv->serviceTypeUrn.c_str());
    ...
    bool res = erroCode(ret, action, response);
    if(res)
    {
